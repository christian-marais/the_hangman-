...
let continuer=1;// la variable sert à interrompre le jeu avec la procédure compter()
let indiceMot=0;// i l
...
function compter(){//procédure qui crée un compteur qui va définir le nombre de tentatives autorisées
    compteur--;
...
function verifierMot(){//procédure qui va verifier la correspondance entre la lettre tapée et les lettres du mots
    ...
    if(continuer!=0){
        for(let a=0;a<tableau[indiceMot].length;a++){
            if(input[a+1].value!=tableau[indiceMot][a]){
                input[a+1].style.background="red";
                input[a+1].style.color="white";
            }else{
                input[a+1].style.background="green";
            }
        }
        compter();
